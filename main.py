...
# ==========================================
# ၃။ SECURE AUTHENTICATION & IMAGES
# ==========================================
...
# ==========================================
# ၄။ NEW: SOCIAL SHARE OPEN GRAPH ENDPOINTS
# ==========================================
...
def share_product(product_id: int, db: Session = Depends(get_db)):
    ...
    if not product: return HTMLResponse("<h1>Product not found</h1>")
    
    vendor_name = product.vendor.store_name if product.vendor.store_name else product.vendor.full_name
    img_url = f"{WEBAPP_URL}/api/image/{product.image_file_id}" if product.image_file_id else "https://via.placeholder.com/800x450"
    target_url = f"{WEBAPP_URL}?start_store={product.vendor_id}"
    ...
    <head>
        <meta charset="UTF-8">
        <title>{product.name} | Digital Mall</title>
        <meta name="description" content="ဈေးနှုန်း: {product.price:,.0f} Ks | {vendor_name} မှ ရောင်းချပါသည်။ ဝင်ရောက်လေ့လာကြည့်ပါ။">
        
        <meta property="og:type" content="product">
        <meta property="og:url" content="{WEBAPP_URL}/share/product/{product_id}">
        <meta property="og:title" content="{product.name} - {product.price:,.0f} Ks">
        <meta property="og:description" content="ဈေးနှုန်း: {product.price:,.0f} Ks | {vendor_name} မှ ရောင်းချပါသည်။ အသေးစိတ်ကို ဝင်ရောက်ကြည့်ရှုပါ။">
        <meta property="og:image" content="{img_url}">
        <meta property="og:image:width" content="800">
        <meta property="og:image:height" content="800">
        
        <script>
            // Automatically redirect users to the app with the specific store open
            window.location.href = "{target_url}";
        </script>
    </head>
    <body style="font-family: sans-serif; text-align: center; padding: 50px;">
        <h2>Redirecting to Digital Mall...</h2>
        <p>အလိုအလျောက်မရောက်ရှိပါက <a href="{target_url}">ဤနေရာကိုနှိပ်ပါ</a></p>
    </body>
    </html>
    """
...
def share_store(vendor_id: int, db: Session = Depends(get_db)):
    vendor = db.query(User).filter(User.id == vendor_id).first()
    if not vendor: return HTMLResponse("<h1>Store not found</h1>")
    
    store_name = vendor.store_name if vendor.store_name else f"{vendor.full_name}'s Store"
    store_addr = f"{vendor.store_township}၊ {vendor.store_state}" if vendor.store_state else "Digital Mall"
    img_url = vendor.store_cover if vendor.store_cover else "https://via.placeholder.com/1280x720.png?text=Welcome+to+Our+Store"
    target_url = f"{WEBAPP_URL}?start_store={vendor_id}"
    ...
    <head>
        <meta charset="UTF-8">
        <title>{store_name} | Digital Mall</title>
        <meta name="description" content="{store_addr} မှ {store_name} ၏ ကုန်ပစ္စည်းများကို ဝင်ရောက်ဝယ်ယူအားပေးနိုင်ပါသည်။">
        
        <meta property="og:type" content="website">
        <meta property="og:url" content="{WEBAPP_URL}/share/store/{vendor_id}">
        <meta property="og:title" content="🏬 {store_name} သို့ ကြိုဆိုပါသည်">
        <meta property="og:description" content="တည်နေရာ: {store_addr}။ ကျွန်ုပ်တို့၏ ဆိုင်ခန်းမှ ပစ္စည်းများကို ယခုပင် ဝင်ရောက်လေ့လာ ဝယ်ယူအားပေးနိုင်ပါသည်။">
        <meta property="og:image" content="{img_url}">
    ...
    <body style="font-family: sans-serif; text-align: center; padding: 50px;">
        <h2>Redirecting to {store_name}...</h2>
        <p>အလိုအလျောက်မရောက်ရှိပါက <a href="{target_url}">ဤနေရာကိုနှိပ်ပါ</a></p>
    </body>
    </html>
    """
...
def get_locations():
    return {
        ...
        # (အခြားဒေသများကို စာသားအရှည်မဖြစ်စေရန် ဤနေရာတွင် အတိုချုံ့ထားပါသည်၊ မူလအတိုင်းထားပါ)
    }
...
async def checkout_cart(req: Request, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    try:
        data = await req.json()
        cart_items = data.get('cart', []); tx_id = data.get('transaction_id', ''); payment_slip_b64 = data.get('payment_slip', '')
        payment_method = data.get('payment_method', 'COD'); address = data.get('address', 'Unknown'); phone = data.get('phone', '')
...
def get_buyer_orders(user: User = Depends(get_current_user), db: Session = Depends(get_db)): return [{"id": o.id, "name": o.product.name, "qty": o.quantity, "price": o.product.price, "status": o.status, "date": o.created_at.strftime("%Y-%m-%d"), "pay": o.payment_method} for o in db.query(Order).filter(Order.user_id == user.id).order_by(Order.created_at.desc()).all()]
...
async def edit_product(product_id: int, request: Request, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    data = await request.json(); product = db.query(Product).filter(Product.id == product_id, Product.vendor_id == user.id).first()
...
# ==========================================
# ၇။ FRONTEND UI WITH SHARE LOGIC
# ==========================================
    ...
    <body class="pb-24">
            ...
            <div class="store-header-fixed bg-white">
                <div id="storefront-cover-img" class="store-cover-container" style="background-image: url('https://via.placeholder.com/800x450');">
                    ...
                    <div class="absolute top-4 left-4 right-4 flex justify-between z-20">
                        <button onclick="closeStore()" class="text-white bg-white/20 hover:bg-white/40 backdrop-blur rounded-full w-10 h-10 flex items-center justify-center font-bold btn-press transition text-xl shadow-sm">&larr;</button>
                        <button onclick="shareStoreAction()" class="text-white bg-white/20 hover:bg-white/40 backdrop-blur rounded-full w-10 h-10 flex items-center justify-center font-bold btn-press transition text-xl shadow-sm">📤</button>
                    </div>
        ...
        <div id="chat-view" class="fixed inset-0 bg-slate-50 z-[60] hidden flex-col">
            <div class="glass-header p-4 flex items-center gap-3 sticky top-0 z-10 shadow-sm">
                ...
                <div class="flex flex-col flex-1"><h2 id="chat-title" class="font-extrabold text-slate-800 text-[15px] truncate">...</h2><span class="text-[10px] text-emerald-500 font-bold flex items-center gap-1"><div class="w-1.5 h-1.5 rounded-full bg-emerald-500"></div> Online Support</span></div>
            </div>
        ...
        <div id="shop-tab" class="tab-content animate-fade-in">
            <div class="px-4 py-3 bg-indigo-50 border-b border-indigo-100 flex flex-col gap-2.5">
                ...
                <div class="flex gap-2"><select id="local-state-filter" onchange="updateLocalTownships(this.value)" class="w-full bg-white border border-indigo-200 text-indigo-700 py-2.5 px-3 rounded-xl text-[13px] font-bold outline-none"><option value="All">နေရာအားလုံး</option></select></div>
                <div class="flex gap-2 hidden" id="advanced-location-filters"><select id="local-township-filter" onchange="triggerSearch()" class="w-1/2 bg-white border border-indigo-200 text-indigo-700 py-2 px-3 rounded-xl text-[12px] font-bold outline-none"><option value="All">မြို့နယ်အားလုံး</option></select><input type="text" id="local-ward-filter" oninput="triggerSearch()" placeholder="ရပ်ကွက် / ကျေးရွာ..." class="w-1/2 bg-white border border-indigo-200 text-slate-700 py-2 px-3 rounded-xl text-[12px] font-bold outline-none"></div>
            </div>
            <div class="p-4 bg-white shadow-sm border-b border-slate-100 mb-3 rounded-b-3xl">
                <div class="relative mb-4"><span class="absolute left-4 top-3 text-slate-400">🔍</span><input type="text" id="search-box" oninput="triggerSearch()" placeholder="ရှာဖွေလိုသော ပစ္စည်းအမည်..." class="w-full py-3 pl-10 pr-4 bg-slate-50 rounded-2xl border border-slate-200 text-sm outline-none font-medium"></div>
        ...
        <div id="orders-tab" class="tab-content hidden p-4 animate-fade-in">
            ...
            <div id="vendor-profile-view" class="hidden animate-fade-up">
                <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 pb-10 mb-5">
                    ...
                    <div class="space-y-4">
                        <div class="bg-slate-50 p-4 rounded-2xl border border-slate-200"><label class="block text-xs font-bold text-slate-700 mb-2">ဆိုင်အဖုံးပုံ (Cover Image)</label><input type="file" accept="image/*" onchange="compressAndEncodeImage(this, 'prof-store-cover', 1280)" class="text-xs text-slate-500 w-full"><input type="hidden" id="prof-store-cover"><img id="prof-store-cover-preview" class="w-full aspect-video object-cover rounded-xl hidden border border-slate-200 mt-3"></div>
                        <div><label class="block text-xs font-bold text-slate-500 mb-1.5">ဆိုင်အမည် *</label><input type="text" id="prof-store-name" class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none"></div>
                        <div><label class="block text-xs font-bold text-slate-500 mb-1.5">တိုင်းဒေသကြီး/ပြည်နယ် *</label><select id="prof-store-state" onchange="updateAddr('prof-store-district', mmData[this.value])" class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold outline-none"></select></div>
                        ...
                        <div><label class="block text-xs font-bold text-slate-500 mb-1.5">ရပ်ကွက် / ကျေးရွာ *</label><input type="text" id="prof-store-ward" class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none"></div>
                        <div><label class="block text-xs font-bold text-slate-500 mb-1.5">အိမ်အမှတ်၊ လမ်းအမည် *</label><input type="text" id="prof-store-street" class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none"></div>
                    </div>
                </div>
                <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 pb-10">
                    ...
                    <div class="space-y-5">
                        <div class="bg-slate-50 p-4 rounded-2xl border border-slate-200"><label class="block text-xs font-extrabold text-indigo-700 mb-2">KPay ဖုန်း / QR</label><input type="text" id="prof-kpay-ph" class="w-full p-3 bg-white rounded-xl border border-slate-200 mb-3 text-sm outline-none"><input type="file" accept="image/*" onchange="compressAndEncodeImage(this, 'prof-kpay-qr', 800)" class="text-xs w-full"><input type="hidden" id="prof-kpay-qr"><img id="prof-kpay-qr-preview" class="h-24 object-cover rounded-xl hidden border border-slate-200 shadow-sm mt-3"></div>
                        <div class="bg-slate-50 p-4 rounded-2xl border border-slate-200"><label class="block text-xs font-extrabold text-amber-600 mb-2">WavePay ဖုန်း / QR</label><input type="text" id="prof-wave-ph" class="w-full p-3 bg-white rounded-xl border border-slate-200 mb-3 text-sm outline-none"><input type="file" accept="image/*" onchange="compressAndEncodeImage(this, 'prof-wave-qr', 800)" class="text-xs w-full"><input type="hidden" id="prof-wave-qr"><img id="prof-wave-qr-preview" class="h-24 object-cover rounded-xl hidden border border-slate-200 shadow-sm mt-3"></div>
                    </div>
        ...
        <script>
            ...
            const WEBAPP_ORIGIN = "{WEBAPP_URL}"; // Dynamic binding for Share functions
            ...
            async function initApp() {{
                ...
                try {{
                    ...
                    // NEW: Deep Link / Auto-Open Logic (Start Parameter Check)
                    let startParam = tg.initDataUnsafe?.start_param || new URLSearchParams(window.location.search).get('start_store');
                    if(startParam) {{
                        // Open specific Store directly 
                        const vendorId = parseInt(startParam);
                        if(!isNaN(vendorId)) {{
                            setTimeout(() => openStore(vendorId), 1000); // slight delay for smooth UI
                        }}
                    }}
            ...
            function performShare(url, title, textMsg) {{
                if (navigator.share) {{
                    navigator.share({{ title: title, text: textMsg, url: url }})
                    .then(() => console.log('Successful share'))
                    .catch((error) => console.log('Error sharing', error));
                }} else {{
                    navigator.clipboard.writeText(url);
                    showToast("✅ Share Link ကို ကူးယူပြီးပါပြီ။");
                }}
            }}

            function shareProductAction(productId, productName) {{
                const shareUrl = WEBAPP_ORIGIN + "/share/product/" + productId;
                performShare(shareUrl, productName, "Digital Mall တွင် ဝယ်ယူအားပေးရန် ဖိတ်ခေါ်ပါသည်။");
            }}

            function shareStoreAction() {{
                if(!currentStoreId) return;
                const shareUrl = WEBAPP_ORIGIN + "/share/store/" + currentStoreId;
                performShare(shareUrl, currentStoreName, "ကျွန်ုပ်တို့၏ ဆိုင်ခန်းလေးကို လာရောက်လေ့လာကြည့်ပါ။");
            }}

            // ... (Chat logic continues to be identically retained) ...
            async function fetchInbox() {{ /*...*/ }}
            function openInboxModal() {{ /*...*/ document.getElementById('inbox-modal').classList.add('active'); fetchInbox(); }}
            ...
            // NEW: Add Share Icon in Product Cards
            function generateProductCardHTML(p, index) {{
                ...
                return `<div class="animate-fade-up bg-white rounded-[24px] shadow-5d overflow-hidden flex flex-col relative transition-all duration-300 transform hover:-translate-y-2 ${{isOut ? 'opacity-60 grayscale-[30%]' : ''}}" style="animation-delay: ${{animDelay}}s">
                    ...
                    <button onclick="shareProductAction(${{p.id}}, '${{p.name.replace(/'/g, "\\'") }}')" class="absolute top-2 right-2 bg-white/80 hover:bg-white backdrop-blur text-slate-700 w-8 h-8 rounded-full flex items-center justify-center z-30 shadow-md transition btn-press">📤</button>
            ...
            // Rendering cart and other logic ...
            function renderGroupedCart() {{ /* Same as before ... */ }}
